 ...
 */
public class MAIN_InvestmentProposalManagementSystem {
    /**
    ...
    private static void demonstrateLambdaExpressions() {
        ...
        Comparator<InvestmentProposal_Class> amountComparator = (p1, p2) ->
        ...
        Predicate<InvestmentProposal_Class> highRiskFilter = proposal -> {
    ...
    private static void demonstrateMethodReferences() {
        ...
        List<InvestmentProposal_Class> proposals = createSampleProposals();
        ...
        Supplier<AbstractClass_InvestmentManager> standardManagerSupplier = StandardAbstractClassInvestmentManager::new;
        AbstractClass_InvestmentManager manager = standardManagerSupplier.get();
    ...
    private static void demonstrateStreamsAPI() {
        ...
        List<InvestmentProposal_Class> highAmountProposals = proposals.stream()
        ...
        List<Double> amounts = proposals.stream()
                .map(InvestmentProposal_Class::getProposedAmount) // Direct field access for efficiency
                .collect(Collectors.toList());
        ...
        double totalInvestment = proposals.stream()
                .mapToDouble(InvestmentProposal_Class::getProposedAmount) // Efficient numeric processing
        ...
        List<InvestmentProposal_Class> sortedByReturn = proposals.stream()
                .sorted(Comparator.comparing(InvestmentProposal_Class::getExpectedReturn).reversed()) // ROI-based prioritization
                .collect(Collectors.toList());
    ...
    private static void demonstrateOptional() {
        ...
        Optional<InvestmentProposal_Class> foundProposal = proposals.stream()
        ...
        String clientName = foundProposal.map(InvestmentProposal_Class::getClientName)
        ...
        Optional<InvestmentProposal_Class> approvedHighReturn = foundProposal
    ...
    private static void demonstrateFunctionalInterfaces() {
        ...
        FunctionalInterface_InvestmentCalculator compoundInterest = (principal, rate, years) ->
        ...
        Predicate<InvestmentProposal_Class> lowRisk = p -> "LOW".equals(p.getRiskLevel()); // Risk-based filtering
        Consumer<InvestmentProposal_Class> approveProposal = p -> p.setApproved(true); // Investment approval action
        Supplier<LocalDateTime> currentTime = LocalDateTime::now; // Timestamp generation for compliance

        InvestmentProposal_Class sample = new InvestmentProposal_Class("TEST-001", "Test Client", 25000, 6.5, "LOW", "STOCKS", "Advisor A");
    ...
    private static void demonstrateDefaultMethods() {
        ...
        interface InvestmentAnalyzer {
            void analyze(InvestmentProposal_Class proposal);
            //
            ...
            default String generateSummary(InvestmentProposal_Class proposal) {
        ...
        InvestmentProposal_Class sample = new InvestmentProposal_Class("ANALYSIS-001", "John Doe", 50000, 7.2, "MEDIUM", "BONDS", "Advisor B");
    ...
    private static void demonstrateAdvancedStreams() {
        ...
        Map<String, List<InvestmentProposal_Class>> byRiskLevel = proposals.stream()
                .collect(Collectors.groupingBy(InvestmentProposal_Class::getRiskLevel)); // Risk-based categorization
        ...
        Map<Boolean, List<InvestmentProposal_Class>> approvedPartition = proposals.stream()
        ...
        DoubleSummaryStatistics stats = proposals.stream()
                .collect(Collectors.summarizingDouble(InvestmentProposal_Class::getProposedAmount)); // Statistical analysis
        ...
        String clientList = proposals.stream()
                .map(InvestmentProposal_Class::getClientName) // Client name extraction
     ...
     */
    private static List<InvestmentProposal_Class> createSampleProposals() {
        return Arrays.asList(
                new InvestmentProposal_Class("PROPOSAL-001", "Alice Johnson", 25000, 6.5, "LOW", "STOCKS", "Advisor A"),
                new InvestmentProposal_Class("PROPOSAL-002", "Bob Smith", 75000, 8.2, "MEDIUM", "BONDS", "Advisor B"),
                new InvestmentProposal_Class("PROPOSAL-003", "Charlie Brown", 150000, 12.0, "HIGH", "REAL_ESTATE", "Advisor C"),
                new InvestmentProposal_Class("PROPOSAL-004", "Diana Prince", 45000, 5.8, "LOW", "MUTUAL_FUNDS", "Advisor A"),
                new InvestmentProposal_Class("PROPOSAL-005", "Eve Wilson", 200000, 15.5, "HIGH", "STOCKS", "Advisor C")
        );
    }
}
